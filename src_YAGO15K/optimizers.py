...
class KBCOptimizer(object):
    ...
    def get_loss(self, batch):
        total_loss_tv2, total_loss_ts, total_loss_vs = self.model.per_sample_loss(batch)
    ...
    def epoch(self, examples: torch.LongTensor, epoch, save_csv=False, is_plot_gmm=False):
        ...
        if epoch >= self.ep:
            total_loss_tv2, total_loss_ts, total_loss_vs = self.get_loss(self.batch_size) # (N, )
            if save_csv:
            ...
            while b_begin < examples.shape[0]:
                ...
                if self.modality_split:
                    if self.fusion_img and self.fusion_dscp:
                        ...
                        preds_img, fac_img, \
                        preds_dscp, fac_dscp, \
                        con_loss, l1_loss = self.model.forward(input_batch, epoch, self.tv2_weights, self.ts_weights, self.vs_weights)
