...
class ComplExMDR(KBCModel):
    ...
    def __init__(
        ...
        if self.fusion_img:
            self.img_dimension = 4096
            self.img_info = pickle.load(open(img_info, 'rb'))
            self.img_vec = torch.from_numpy(self.img_info).float().cuda().squeeze()
    ...
    def score(self, x):
        ...
        if self.modality_split:
            if self.fusion_img:
                mask_img = self.sparse_img(self.img_vec)  # (14541, 4000)
                img_embeddings = (self.img_vec).mm(self.img_post_mats)  # (14541, 4000)
    ...
    def forward(self, x, epoch, tv2_weights, ts_weights, vs_weights):
        lhs = self.r_embeddings[0](x[:, 0])
        rel = self.r_embeddings[1](x[:, 1])
        rhs = self.r_embeddings[0](x[:, 2])
        lhs_temp = lhs
        ...
        if epoch < self.ep:
            ...
            con_loss = align_loss_tv2 + align_loss_ts + align_loss_vs
        else:
            ...
            con_loss = (align_loss_tv2 + align_loss_ts + align_loss_vs) / 3
    ...
    def per_sample_loss(self, batch_size):
        ...
        return total_loss_tv2, total_loss_ts, total_loss_vs
