...
from models import CP, ComplEx, ComplExMDR
...
big_datasets = ['FB15K-237', 'WN18RR', 'YAGO15K']
...
model = {
    ...
    'ComplExMDR': lambda: ComplExMDR(dataset.get_shape(), args.rank, args.init, args.modality_split, args.fusion_img,
                                     ...
                                     args.dataset, args.scale, args.img_info, args.dscp_info, args.ep, args.temp)
}[args.model]()
...
optimizer = KBCOptimizer(model, regularizer, optim_method, args.batch_size, args.modality_split, args.fusion_img,
                         args.fusion_dscp, size=dataset.get_shape()[0], ep=args.ep)
...
for e in range(args.max_epochs):
    ...
    # ]
    cur_loss = optimizer.epoch(examples, e).tolist()
    ...
    if (e + 1) % args.valid == 0:
        ...
        scheduler.step(valid['hits@[1,3,10]'][-1])
